...
if(isset($_GET['charge_id']) && !empty($_GET['charge_id']) || !empty($check_merchant['fetch']['id'])){
    ...
    $query = array(
        ...
        appSubscriptionCreate(
            name: "'.getenv('APPLICATION_NAME').' Recurring Charge"
            lineItems: {
                plan: {
                    appRecurringPricingDetails: {
                        price: {
                                amount: '. getenv("APPLICATION_PRICE") .'
                                currencyCode: USD
                        }
                    }
                }
    ...
    echo "<script type='text/javascript'> top.window.location='". $charge['data']['appSubscriptionCreate']['confirmationUrl'] ."'</script>";
    die();
}